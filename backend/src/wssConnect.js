...
const sendUser = (payload, ws) => {
    sendData(['setUsers', payload], ws);
}

const sendMessage = (payload, ws) => {
    sendData(['updateMessages', payload], ws);
}

const sendRoom = (payload, ws) => {
    sendData(['creatRoom', payload], ws);
}

const sendClearInfo = (payload, ws) => {
    sendData(['cleared', payload], ws);
}
...
const signUp = async (payload, ws, channels) => {
    ...
    if(user){
        ...
        for(let key in channels){
            sendUser(users.map(e => e.username), channels[key]);
        }
    }
};
...
const signIn = async (payload, ws, channels) => {
    ...
    if(user&&await bcrypt.compare(password, user.password)){
        ...
        sendStatus({ type: 'success', msg: `${username}, Welcome to Bongo Chat!` }, ws);
        sendData(['signIn', userRooms], ws);
        sendUser(users.map(e => e.username), channels[username]);
        return true;
    }else{
...
const createRoom = async (payload, channels) => {
    ...
    payload.members.forEach(e => {
        if(channels[e]){
            ...
            else sendStatus({ type: 'success', msg: `${creator} invite you to Bongo Cha Cha Cha!` }, channels[e]);            
        }        
    })
}
...
const input = async (payload, channels) => {
    ...
    room.members.forEach(e => {
        if(channels[e]){
            sendMessage(payload, channels[e]);
...
const clear = async (payload, channels) => {
    ...
    room.members.forEach(e => {
        if(channels[e]){
            sendClearInfo(payload, channels[e]);
